...
XFormView::XFormView(QWidget *parent)
    : ArthurFrame(parent),
      ...
      m_rotation(0.0),
      m_text(),
      m_pixmap(QPixmap(":/bg1.jpg")),
      m_points(new HoverPoints(this, HoverPoints::CircleShape)),
      m_ctrlPoints()
{
    setAttribute(Qt::WA_MouseTracking);

    m_points->setConnectionType(HoverPoints::LineConnection);
    m_points->setEditable(false);
    m_points->setPointSize(QSize(15, 15));
    m_points->setShapeBrush(QBrush(QColor(151, 0, 0, alpha)));
    m_points->setShapePen(QPen(QColor(255, 100, 50, alpha)));
    m_points->setConnectionPen(QPen(QColor(151, 0, 0, 50)));
    m_points->setBoundingRect(QRectF(0, 0, 500, 500));
    m_ctrlPoints << QPointF(250, 250) << QPointF(350, 250);
    connect(m_points, SIGNAL(pointsChanged(QPolygonF)),
...
void XFormView::drawVectorType(QPainter *p)
{
    QPainterPath path;
    p->translate(m_ctrlPoints.at(0) - QPointF(250, 250));

    p->scale(0.77, 0.77);
    p->translate(98.9154 + 30, -217.691 - 20);
    ...
    p->setBrush(QColor( 193, 193, 191, 255));
    ...
    p->setBrush(QColor(183, 114, 0, 255));
    ...
    p->setBrush(QColor(242, 183, 0, 255));
...
void XFormView::drawPixmapType(QPainter *p)
{
    QPointF center(m_pixmap.width() / qreal(2), m_pixmap.height() / qreal(2));
    p->translate(m_ctrlPoints.at(0) - center);
    ...
    p->setPen(QPen(QColor(255, 0, 0, alpha), 0.25, Qt::SolidLine, Qt::FlatCap, Qt::BevelJoin));
    p->setBrush(Qt::NoBrush);
    p->drawRect(QRectF(0, 0, m_pixmap.width(), m_pixmap.height()).adjusted(-2, -2, 2, 2));
}

void XFormView::drawTextType(QPainter *p)
{
    QPainterPath path;
...
{
    m_points->setBoundingRect(rect());
    ArthurFrame::resizeEvent(e);
}

void XFormView::setType(XFormView::XFormType t)
{
    m_type = t;
    update();
}
...
void XFormView::setRotation(qreal r)
{
    ...
    QPointF center = m_points->points().at(0);
    QMatrix m;
    m.translate(center.x(), center.y());
    m.rotate(m_rotation - oldRotation);
    m.translate(-center.x(), -center.y());
    m_points->setPoints(m_points->points() * m);
...
{
    QPointF translate = points.at(0) - m_ctrlPoints.at(0);

    if (qAbs(points.at(0).x() - points.at(1).x()) < 10
            && qAbs(points.at(0).y() - points.at(1).y()) < 10) {
        m_points->setPoints(m_ctrlPoints);
    }
    if (!translate.isNull()) {
        m_ctrlPoints[0] = points.at(0);
        m_ctrlPoints[1] += translate;
        m_points->setPoints(m_ctrlPoints);
    }
    m_ctrlPoints = points;
...
void XFormView::reset()
{
    ...
    emit shearChanged(0);
    m_ctrlPoints = QPolygonF();
    m_ctrlPoints << QPointF(250, 250) << QPointF(350, 250);
    m_points->setPoints(m_ctrlPoints);
    m_points->firePointChange();
}
...
void XFormView::timerEvent(QTimerEvent *e)
{
    if (e->timerId() == m_timer.timerId()) {
        ...
        static qreal scaleIncrement = 0.003;
        static qreal shearIncrement = -0.001;
        emit scaleChanged(int((m_scale + scaleIncrement) * 1000));
        emit shearChanged(int((m_shear + shearIncrement) * 1000));
      ...
      m_textEditor(new QLineEdit)
{
    ...
    enableOpenGLButton->setChecked(m_view->usesOpenGL());
    ...
    typeGroupLayout->addWidget(m_textEditor);
    ...
    connect(rotateSlider, &QSlider::valueChanged, m_view, &XFormView::changeRotation);
    connect(scaleSlider, &QSlider::valueChanged, m_view, &XFormView::changeScale);
    connect(shearSlider, &QSlider::valueChanged, m_view, &XFormView::changeShear);

    connect(vectorType, SIGNAL(clicked()), m_view, SLOT(setVectorType()));
    connect(pixmapType, SIGNAL(clicked()), m_view, SLOT(setPixmapType()));
    connect(textType, SIGNAL(clicked()), m_view, SLOT(setTextType()));

    connect(m_view, &XFormView::rotationChanged, rotateSlider, &QSlider::setValue);
    connect(m_view, &XFormView::scaleChanged, scaleSlider, &QSlider::setValue);
    connect(m_view, &XFormView::shearChanged, shearSlider, &QSlider::setValue);

    connect(resetButton, SIGNAL(clicked()), m_view, SLOT(reset()));
    connect(animateButton, &QPushButton::clicked, m_view, &XFormView::setAnimation);
    connect(whatsThisButton, &QPushButton::clicked, m_view, &XFormView::setDescriptionEnabled);
    connect(whatsThisButton, &QPushButton::clicked, m_view->hoverPoints(), &HoverPoints::setDisabled);
    connect(m_view, &XFormView::descriptionEnabledChanged, m_view->hoverPoints(), &HoverPoints::setDisabled);
    connect(m_view, &XFormView::descriptionEnabledChanged, whatsThisButton, &QPushButton::setChecked);
    connect(showSourceButton, SIGNAL(clicked(bool)), m_view, SLOT(showSource()));
#ifdef QT_OPENGL_SUPPORT
    connect(enableOpenGLButton, &QPushButton::clicked, m_view, &XFormView::enableOpenGL);
#endif
    m_view->loadSourceFile(":/xform.cpp");
    m_view->loadDescription(":/xform.html");

    // defaults
    m_view->reset();
    vectorType->setChecked(true);
    m_textEditor->setText("Qt Affine Transformations Example");
